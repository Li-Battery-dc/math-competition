...
def initial_solution(data_points):
    while True:
        solution = random.sample(data_points, 4)
        if check_coverage(solution):
            return solution

def check_coverage(solution):
    # 检查解是否在每个索引上覆盖了0, 1, 2, 3
    indices = ['a', 'b', 'c', 'd', 'e', 'f', 'g']  # 索引名列表
    for idx in indices:
        if set(point[idx] for point in solution) != {0, 1, 2, 3}:
            return False
    return True

def calculate_cost(solution):
    weights = sum(point['weight'] for point in solution)
    values = [point['value'] for point in solution]
    if max(values) - min(values) > 5:
        return float('inf')
    return weights

def generate_neighbor(solution, data_points):
    new_solution = solution[:]
    index_to_replace = random.randint(0, 3)
    # 确保新解也满足索引覆盖条件
    while True:
        new_point = random.choice(data_points)
        new_solution[index_to_replace] = new_point
        if check_coverage(new_solution):
            return new_solution

def simulated_annealing(data_points):
    current_solution = initial_solution(data_points)
    ...
    while T > T_min:
        new_solution = generate_neighbor(current_solution, data_points)
...
best_solution, best_cost = simulated_annealing(data_points)
print("Best Solution:", best_solution)
print("Minimum Weight Sum:", best_cost)
