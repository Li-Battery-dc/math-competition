    ...
    for idx2 in range(4):
        for idx3 in range(4):
            for idx4 in range(4):
                ...
                # print("第",code,"个假设值为",result_combination[code,:])
                code+=1
 # 第三步：验证和求解最优解
...
for index in range(256):
    ...
    if(res_min<=50):
        print("第",index,"种假设的验证时间残差为",res_min)
        print("该结果使用的四个标准机时间索引",standard_code_combination[index])
        print("该结果的检验机时间索引为",validation_code_combination[index])
        print("该假设的结果为",result_combination[index])
        print("/n")
        code_combination=np.append(standard_code_combination[index],validation_code_combination[index],axis=0)
        '''
        matrix=np.zeros((7,4))
        for i in range(0,4):
            for j in range(0,3):
                matrix[i][j]=data[i][j]
            matrix[i][3]=data[int(standard_code_combination[index,i])]
        print(matrix)
        '''
