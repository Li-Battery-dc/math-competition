...
# 将经纬度转换为大致的米
coords[:, 0] *= 111320  # 经度每度的米
coords[:, 1] *= 110574  # 纬度每度的米
...
for idx in range(256):
    combination = np.unravel_index(idx, (4, 4, 4, 4))
    picked_times = standard_times[np.arange(4), combination]
    initial_guess = [0, 0, 0, 0]  # 初始猜测值
    result = least_squares(residuals, initial_guess, args=(standard_machines, picked_times))
...
def residuals(params, coords, fact_times):
    ...
    times=[theoretical_times,theoretical_times,theoretical_times]
    return np.sum((times - fact_times)**2)
    ...
    for idx1 in range(4):
        for idx2 in range(4):
            for idx3 in range(4):
                ...
                temp_res[temp_code]=residuals(result_combination[index,:],validation_machines,[t1,t2,t3])
                temp_code+=1
