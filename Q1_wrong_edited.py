...
while flag:
    ...
    x = np.array(data[[0,1,2,3],0])  # 接收器的x坐标
    y = np.array(data[[0,1,2,3],1])  # 接收器的y坐标
    z = np.array(data[[0,1,2,3],2])  # 接收器的z坐标
    t = np.array(data[[0,1,2,3],3])  # 接收到信号的时间
    ...
    def error_square_sum0(x0):
        ...
        for row in data0:
            ...
            xi, yi, zi, ti = row
            distance=((xi - xa0)**2 + (yi - ya0)**2 + (zi - za0)**2)**0.5
            predicted_time=distance/c
    ...
    for index in range(0,7):
        temp_distance=np.sqrt((xa-data[index,0])**2+(ya-data[index,1])**2+(za-data[index,2])**2)
        temp_predicted_time=temp_distance/c
        residual=(data[index,4]-(ta+temp_predicted_time))**2
        residuals.append(residual)
    for idx in range(0,7):
        if residuals[idx]>1000:
            mask[idx]=0
            flag_remove=True
            print("移除第",idx,"个点")
